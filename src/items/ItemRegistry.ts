...
export class ItemRegistry {
  ...
  register(item: ItemDefinition): this {
    ...
    if (item.placeBlockId !== undefined && this.blockItems.has(item.placeBlockId)) {
      throw new Error(`Block ${item.placeBlockId} already has an item mapping.`);
    }
    const frozen = Object.freeze({ ...item, tool: item.tool ? Object.freeze({ ...item.tool }) : undefined });
  ...
  getItemIdForBlock(blockId: number): number | undefined {
    return this.blockItems.get(blockId);
  }
